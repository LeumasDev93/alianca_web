...
interface DestaquesDataDetailhes {
  titulo: string;
  description: string;
  descricao_2: string;
  descricao: string;
  subTitle: string;
  paragrafo_1: string;
  imagem_1?: DestaqueImage[];
  banner_imagem?: DestaqueImage[];
  destaque?: {
    titulo: string;
    description: string;
    url_botton: string;
      ...
      {loadingButtons[topico.id] ? (
        ...
        <>
          <div className="relative w-16 h-full flex-shrink-0">
            {topico.imagem && (
              <Image
                src={`${BASE_IMAGE_URL}${topico.imagem.url}`}
                alt={topico.titulo}
                width={100}
                height={100}
                quality={100}
...
export default function DestaqueDetails() {
  const params = useParams(); // Captura os parâmetros da rota
  const id = params.id; // Obtém o `id` da URL
  console.log("id", id);

  const [card, setCard] = useState<DestaquesDataDetailhes[]>([]);
  ...
  const fetchDatas = async () => {
    ...
    try {
      ...
      const response = await apiAlianca.get<
        APIResponse<DestaquesDataDetailhes>
      >(`/detalhes-destaques?filters[destaque][id][$eq]=${id}&populate=*`);
  ...
  const fetchTopicos = async (page: number) => {
    try {
      const response = await apiAlianca.get(
        `/destaques?populate=*&pagination[page]=${page}&pagination[pageSize]=4&filters[id][$ne]=${id}`
      );
